...
class SecretConsultationOnBoardingProvider with ChangeNotifier {
  int _currentPage = 0;
  ...
  Future<void> next(BuildContext context) async {
    _currentPage++;
    if(currentPage > secretLawyerOnBoardingData.length - 1) {
      skip(context);
    }
    else {
  ...
  Future<void> skip(BuildContext context) async {
    CacheHelper.setData(key: PREFERENCES_KEY_IS_FIRST_OPEN_SECRET_LAWYER, value: false);
    Navigator.pushReplacementNamed(context, Routes.secretConsultationFormRoute);
  }
}